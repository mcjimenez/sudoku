var sudoku = (function() {
  ...
  var BTO_FIX = 'btoFix';
  var BTO_FIX_SEL = 'btoFixSel';
  var BTO_EMP = 'btoEmp';
  var BTO_EMP_SEL = 'btoEmpSel';
  var BTO_STF_SEL = 'btoStfSel';
  var BTO_STF = 'btoStf';
  var BTO_ERR_SEL = 'btoErrSel';
  var BTO_ERR = 'btoErr';

  var TXT_BTO_CLEAN_POSITION = 'Clean position';
  var TXT_BTO_CLN_ALL = 'Clean all';
  var TXT_BTO_NEW_BOA = 'New board';

  var BTO_CLN_ALL = 'cleanAll';
  var BTO_NEW_BOA = 'newBoard';
  var BTO_CLEAN_POSITION = 'cleanPos';
  ...
  var resetBoard = function() {
    ...
    for (var i = 0, length = initVal.length; i < length; i++) {
      ...
      board[initVal[i].substr(0, 1)][initVal[i].substr(1, 1)] =
          initVal[i].substr(2, 1);
    ...
    xhr.onreadystatechange = function(ex) {
      if (xhr.readyState === 4) {
        if (xhr.status === 0 || xhr.status === 200) {
          ...
          var rInf = 0;
          var ran = Math.floor(Math.random() * (nEltos - rInf)) +
                    parseInt(rInf);
  ...
  var changeSelectElto = function(newCel, sel) {
    ...
    var clsNewValC;
    for (var i = 0; i < 9; i++) {
      clsValF = document.getElementById(f + i.toString()).attributes
                .getNamedItem('class');
      clsValC = document.getElementById(i.toString() + c).attributes
                .getNamedItem('class');
      if (sel) {
  ...
  function countPieceOk(oldType, isNewValCorrect) {
    if (isNewValCorrect) {
      if (oldType.substr(0, 6) === BTO_EMP ||
          oldType.substr(0, 6) === BTO_ERR) {
        numPieceOk = numPieceOk + 1;
      }
    } else {
  ...
  var setValue = function(newVal) {
    ...
    default:
      ...
      verifyOthersSquare(board, parseInt(indFicha.substr(0, 1)),
                         parseInt(indFicha.substr(1, 1)));
      break;
    }
    changeSelectElto(square, false);
    square = null;
  ...
  var isValOk = function(newVal, b, fil, col) {
    ...
    var f = (fil !== undefined ? fil : square.attributes.getNamedItem('name')
        .value.substr(0, 1));
    var c = (col !== undefined ? col : square.attributes.getNamedItem('name')
        .value.substr(1, 1));
    var ok = true;
    //Verifies that the Global file and column don't already have the new value
  ...
  var setBtoTxt = function() {
    document.getElementById(BTO_CLEAN_POSITION).childNodes[0].nodeValue =
        TXT_BTO_CLEAN_POSITION;
    document.getElementById(BTO_CLN_ALL).childNodes[0].nodeValue =
        TXT_BTO_CLN_ALL;
    document.getElementById(BTO_NEW_BOA).childNodes[0].nodeValue =
        TXT_BTO_NEW_BOA;
  };
  ...
  function solve(board, posX, posY, solutions, ite) {
    ...
    if (board[posX][posY]) {
      ...
      try {
        ...
        } while (solutions.numSol < solutions.maxNumSol &&
